...
function renderChatPanel(chat, messages) {
    ...
    panel.innerHTML = `
        <header class="chat-panel-header">
            ...
            <div class="chat-panel-actions">
                <button class="chat-panel-btn" ${chat.listing_id ? `onclick="window.location.href='${BASE_URL}/listings/${escapeHtml(chat.listing_id)}'"` : ''}>View listing</button>
                <button class="chat-panel-btn" ${chat.application_id ? `onclick="window.location.href='${BASE_URL}/applications/${escapeHtml(chat.application_id)}'"` : ''}>View exchange details</button>
            </div>
        </header>
