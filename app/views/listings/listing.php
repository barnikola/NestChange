...
<section class="holder">
        ...
        <div class="carousel-track">
            <?php if (!empty($listing['images'])): ?>
                <?php foreach ($listing['images'] as $index => $img): ?>
                    <img src="/<?php echo ltrim($img['image'], '/'); ?>"
    ...
    <div class="text-box">
        ...
        <p class="listing-location"><?php echo htmlspecialchars($listing['city']); ?>,
            <?php echo htmlspecialchars($listing['country']); ?>
        </p>
...
<section class="host-panel">
    ...
    <div class="host-actions">
        ...
        <a href="<?php echo BASE_URL; ?>/chat/<?php echo htmlspecialchars($listing['host_profile_id']); ?>?listing_id=<?php echo htmlspecialchars($listing['id']); ?>"
            class="host-action-btn primary">
            Contact Host
        </a>
    </div>
</section>
...
<section class="calendar-card">
    ...
    <div class="listing-calendar">
        <div id="availability-calendar" data-year="<?php echo date('Y'); ?>" data-month="<?php echo date('n'); ?>"
    ...
    <?php if (!empty($listing['availability'])): ?>
        <div class="availability-info"
            style="margin-top: 20px; padding-top: 16px; border-top: 1px solid #f0f0f0; font-size: 0.9em; color: #666;">
...
<div class="div-button">
    <?php
    require_once dirname(__DIR__, 2) . '/helpers/ProfileHelper.php';
    if (ProfileHelper::canApply()):
        ?>
        <button class="listing"
            onclick="window.location.href='<?php echo BASE_URL; ?>/listings/<?php echo $listing['id']; ?>/apply'">
            Apply for Exchange
        </button>
    <?php else: ?>
        <button class="listing" disabled style="opacity: 0.5; cursor: not-allowed; background-color: #666;">
            Apply for Exchange (Verified Only)
        </button>
        <p style="text-align: center; margin-top: 10px; font-size: 0.9rem; color: #666;">
            Please <a href="/profile" style="color: #000; text-decoration: underline;">verify your profile</a> to apply.
        </p>
    <?php endif; ?>
</div>
</section>
